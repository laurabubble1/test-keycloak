...
SHARED_REALM = "loadtest-realm"


class KeycloakUser(HttpUser):
    wait_time = between(2, 5)  # Think time between requests (2-5 seconds)
    # host will be set by Docker command line argument

    def on_start(self):
        self.token_expires_at = 0
        self.test_realm_name = SHARED_REALM
        self.created_resources = {"users": [], "clients": [], "roles": [], "groups": [], "realms": []}
        self.get_admin_token()
        print(
            f"[{self.environment.runner.worker_id if hasattr(self.environment.runner, 'worker_id') else 'MASTER'}] Using shared realm: {self.test_realm_name}"
        )
        self.create_test_realm(shared_realm=SHARED_REALM)
    ...
    def get_admin_token(self):
        ...
        response = self.client.post(
            token_url, data=token_data, name="auth/get_admin_token"
        )
        if response.status_code == 200:
            token_info = response.json()
            self.admin_token = token_info["access_token"]
            self.token_expires_at = time.time() + (
                token_info.get("expires_in", 300) - 60
            )
    ...
    def create_test_realm(self, shared_realm="loadtest-realm"):
        """Ensure the shared realm exists"""
        self.test_realm_name = shared_realm
        
        # Check if realm exists
        check_response = self.client.get(f"/admin/realms/{shared_realm}", name="check/realm_exists")
        
        if check_response.status_code == 404:
            # Realm doesn't exist, create it
            realm_data = {
                "realm": shared_realm,
                "displayName": f"Load Test Realm - {shared_realm}",
                "enabled": True
            }
            create_response = self.client.post("/admin/realms", json=realm_data, name="setup/create_shared_realm")
            if create_response.status_code == 201:
                print(f"✓ Created shared realm: {shared_realm}")
            else:
                print(f"✗ Failed to create realm: {create_response.status_code}")
        elif check_response.status_code == 200:
            print(f"✓ Using existing shared realm: {shared_realm}")
        else:
            print(f"⚠️ Unexpected response checking realm: {check_response.status_code}")
            
        self.created_resources["realms"].append(shared_realm)
    ...
    def cleanup_resources(self):
        ...
        if self.test_realm_name in self.created_resources["realms"]:
            self.client.delete(
                f"/admin/realms/{self.test_realm_name}",
                name="cleanup/delete_test_realm",
            )
    ...
    def create_user(self):
        ...
        response = self.client.post(
            f"/admin/realms/{self.test_realm_name}/users",
            json=user_data,
            name="create/user",
        )
    ...
    def create_client(self):
        ...
        response = self.client.post(
            f"/admin/realms/{self.test_realm_name}/clients",
            json=client_data,
            name="create/client",
        )
        if response.status_code == 201:
    ...
    def create_realm_role(self):
        ...
        response = self.client.post(
            f"/admin/realms/{self.test_realm_name}/roles",
            json=role_data,
            name="create/realm_role",
        )
        if response.status_code == 201:
    ...
    def create_group(self):
        ...
        response = self.client.post(
            f"/admin/realms/{self.test_realm_name}/groups",
            json=group_data,
            name="create/group",
        )
        if response.status_code == 201:
    ...
    def create_client_scope(self):
        ...
        self.client.post(
            f"/admin/realms/{self.test_realm_name}/client-scopes",
            json=scope_data,
            name="create/client_scope",
        )
    ...
    def read_clients(self):
        ...
        self.ensure_valid_token()
        self.client.get(
            f"/admin/realms/{self.test_realm_name}/clients", name="read/clients"
        )
    ...
    def read_realm_roles(self):
        ...
        self.ensure_valid_token()
        self.client.get(
            f"/admin/realms/{self.test_realm_name}/roles", name="read/realm_roles"
        )
    ...
    def read_groups(self):
        ...
        self.ensure_valid_token()
        self.client.get(
            f"/admin/realms/{self.test_realm_name}/groups", name="read/groups"
        )
    ...
    def read_realm_info(self):
        ...
        self.ensure_valid_token()
        self.client.get(f"/admin/realms/{self.test_realm_name}", name="read/realm_info")
    ...
    def update_user(self):
        ...
        if self.created_resources["users"]:
            ...
            self.client.put(
                f"/admin/realms/{self.test_realm_name}/users/{user_id}",
                json=update_data,
                name="update/user",
            )
    ...
    def update_client(self):
        ...
        if self.created_resources["clients"]:
            ...
            self.client.put(
                f"/admin/realms/{self.test_realm_name}/clients/{client_id}",
                json=update_data,
                name="update/client",
            )
    ...
    def update_realm_role(self):
        ...
        if self.created_resources["roles"]:
            ...
            get_response = self.client.get(
                f"/admin/realms/{self.test_realm_name}/roles/{role_name}",
            ...
            if get_response.status_code == 200:
                ...
                current_role["description"] = (
                    f"Updated role description - {int(time.time())}"
                )
                self.client.put(
                    f"/admin/realms/{self.test_realm_name}/roles/{role_name}",
                    json=current_role,
    ...
    def update_group(self):
        ...
        if self.created_resources["groups"]:
            ...
            get_response = self.client.get(
                f"/admin/realms/{self.test_realm_name}/groups/{group_id}",
            ...
            if get_response.status_code == 200:
                ...
                current_group["attributes"]["description"] = [
                    f"Updated group - {int(time.time())}"
                ]
    ...
    def delete_client_scope(self):
        ...
        if response.status_code == 200:
            ...
            test_scopes = [
                s for s in scopes if s.get("name", "").startswith("test-scope-")
            ]
            if test_scopes:
                scope_id = random.choice(test_scopes)["id"]
                self.client.delete(
                    f"/admin/realms/{self.test_realm_name}/client-scopes/{scope_id}",
